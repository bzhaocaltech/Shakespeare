# The base code is taken from set 6 solutions, however several notable revisions
...
class HiddenMarkovModel:
    ...
    def __init__(self, A, O, int_word, word_int):
        '''
        ...
        Arguments:
            ...
            int_word:   A dictionary for converting ints into words
...
def unsupervised_HMM(training_sequences, n_states, N_iters):
    '''
    ...
    Arguments:
        training_sequences:
                    A dataset consisting of input sequences in the form
                    of lists of variable length, consisting of words
                    ranging from 0 to D - 1. In other words, a list of lists.
    ...
    # Create a dictionary that maps words to ints and vice versa
    current_word_num = 0
    int_word = {}
    word_int = {}
    for line in training_sequences:
        for word in line:
            if word not in word_int:
                word_int[word] = current_word_num
                int_word[current_word_num] = word
                current_word_num += 1

    # Convert line data into its corresponding ints using word_int
    X = []
    for line in training_sequences:
        int_line = []
        for word in line:
            int_line.append(word_int[word])
        X.append(int_line)
    ...
    HMM = HiddenMarkovModel(A, O, int_word, word_int)
